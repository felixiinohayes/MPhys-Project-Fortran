...
!----------------------------------------------------
    ...
    integer*4 i,j,k,nr,ir,i1,i2,lwork,info,ikx,iky,j1,j2,nb,l
    real*8,parameter::third=1d0/3d0,alpha=1d0,Bx=0.5d0,pi_8=4*atan(1.0_8),cos60=cos(pi_8/3),tan30=tan(pi_8/6),sin30=sin(pi_8/6),beta0=3d0
    real*8 phase,pi2,jk,a,b,x1,y1,theta_R,beta
    real*8 xk(nk),bvec(3,3),avec(3,3),ktemp1(3),ktemp2(3),xkl(nkpath),kpoints(3,nkpath),kpath(3,nk),dk(3)
    real*8,allocatable:: rvec(:,:),rvec_data(:,:),ene(:,:),rwork(:),od(:,:,:),rvec_cart(:,:)
...
!-------------kpath
    ...
    ! kpoints(:,2) = [  0.00d0,  0.0d0,  0.5d0 ]  !A
...
!----read H(R) 
    ...
    read(99,*)
    read(99,*)nb,nr
    allocate(rvec(3,nr),rvec_data(3,nr),rvec_cart(3,nr),Hk(nb,nb),triv_hr(nb,nb,nr),top_hr(nb,nb,nr),ndeg(nr),ene(nb,nk),od(nb,nk,3),H_col(nb))
    read(99,*)ndeg
    do i = 1, 80
...
!----------Add mechanical stress
	do ir=1,nr
    	do i=1,6
        	do j=1,6
				rvec_cart(:,ir) = rvec(1,ir)*avec(:,1) + rvec(2,ir)*avec(:,2)
	...
	! do ir=1,1100
    	! do i=1,nb
        	! do j=1,nb
	! 			if(abs(top_hr(2,1,ir) - conjg(top_hr(1,2,ir))) > 0.3) print *, "NOT MATCHING", ir, top_hr(2,1,ir),top_hr(1,2,ir)
	! 		enddo
	! 	enddo
	! enddo
...
!---Orbital probability:
    ...
    do i=1,nb
       ...
       enddo
         ...
         write(100,*)
    enddo
    call write_plt(nkpath,xkl,klabel,ef,Bx,beta0)
    stop
...
444 write(*,'(3a)')'ERROR: input file "',trim(adjustl(triv_file)),' not found'
    ...
    subroutine write_plt(nkp,xkl,kl,ef,Bx,beta0)
          implicit none
          integer nkp,i
          real*8 xkl(nkp),ef,Bx,beta0
          ...
          write(99, '(a,f12.3,a,f12.3,a)') 'set title "B =',Bx,'T (along y-axis) \n beta_0 =',beta0,' (along y-axis)"'
          write(99,'(13(a,/),a)') &
               'set border',&
               'set xtics',&
               'set ytics',&
