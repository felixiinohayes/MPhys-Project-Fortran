...
!--------to be modified by the user
    ...
    real*8,parameter::ef= 4.18903772,kmax=0.000035,a=0.791,diff_value=kmax/10
...
!----- Create header of dx files
  ...
  !----- Create a uniform k-mesh
	...
	allocate(HK(nb,nb),H_k(nb,nb,ikx,iky,ikz),k_ene(nb),eig(4,4,nb,nkpoints,nkpoints,nkpoints))
    allocate(H_eff(2,2),eig_eff(2,2,nkpoints,nkpoints,nkpoints),eff(2),du(nb,3))
	allocate(connection(4,3,nkpoints,nkpoints,nkpoints),curvature(3,nkpoints,nkpoints,nkpoints))
...
!----- Berry connection
	...
	do ikx=1,nkpoints
		do iky=1,nkpoints
			do ikz=1,nkpoints
				do ik=1,4
					du(:,1) = (eig(ik,2,:,ikx,iky,ikz) - eig(ik,1,:,ikx,iky,ikz))/diff_value
					du(:,2) = (eig(ik,3,:,ikx,iky,ikz) - eig(ik,1,:,ikx,iky,ikz))/diff_value
					du(:,3) = (eig(ik,4,:,ikx,iky,ikz) - eig(ik,1,:,ikx,iky,ikz))/diff_value

					connection(ik,1,ikx,iky,ikz) = dot_product(eig(ik,1,:,ikx,iky,ikz),du(:,1))
					connection(ik,2,ikx,iky,ikz) = dot_product(eig(ik,1,:,ikx,iky,ikz),du(:,2))
					connection(ik,3,ikx,iky,ikz) = dot_product(eig(ik,1,:,ikx,iky,ikz),du(:,3))
				enddo
				! write(100, '(3(1x,f20.8))') connection(3,:,ikx,iky,ikz)
...
!----- Berry curvature
	...
	do ikx=1,nkpoints
		do iky=1,nkpoints
			do ikz=1,nkpoints
				dAdx(:) = (connection(2,:,ikx,iky,ikz) - connection(1,:,ikx,iky,ikz))/diff_value
				dAdy(:) = (connection(3,:,ikx,iky,ikz) - connection(1,:,ikx,iky,ikz))/diff_value
				dAdz(:) = (connection(4,:,ikx,iky,ikz) - connection(1,:,ikx,iky,ikz))/diff_value
				! print *, dAdx(2)-dAdy(1),ikx,iky,ikz

				curvature(1,ikx,iky,ikz) = real((dAdy(3) - dAdz(2))) ! Compute curl
				curvature(2,ikx,iky,ikz) = real((dAdz(1) - dAdx(3)))
				curvature(3,ikx,iky,ikz) = real((dAdx(2) - dAdy(1)))			
