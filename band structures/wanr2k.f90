...
!--------to be midified by the usere
    character(len=80):: prefix="../BiTeI",files="ortho"
...
!------------------------------------------------------
    ...
    real*8,parameter::third=1d0/3d0, a = 0
    real*8 phase,pi2,jk,x1,y1
    ...
    complex*16,allocatable:: Hk(:,:),work(:),H_col(:),top_Hr_temp(:,:),triv_Hr_temp(:,:),B_pt(:,:)
...
!       data kpoints(:,3) /     third,      third,    0.5d0/  !H
    ...
    data kpoints(:,1) /     0.5d0,      0.0d0,    0.5d0/  !L
    data kpoints(:,2) /     0.0d0,      0.0d0,    0.5d0/  !A
    data kpoints(:,3) /     -0.5d0,      0.0d0,    0.5d0/  !H
            ...
            do iy=-6,6
                do iz=-6,6
                    if(ndeg(ix,iy,iz).ne.0) then
                        ...
                        Hk=Hk+(interp_Hr(:,:,ix,iy,iz))*dcmplx(cos(phase),-sin(phase))/float(ndeg(ix,iy,iz))
                    endif
                enddo
            enddo
        enddo
...
!-----Fermi level:
      ef = (MAXVAL(ene(nb/2, :)) + MINVAL(ene(nb/2+1, :)))/2.0d0
      eg = MINVAL(ene(nb/2+1, :)) - MAXVAL(ene(nb/2, :))
...
444   write(*,'(3a)')'ERROR: input file "',trim(adjustl(triv_file)),' not found'
    ...
    subroutine write_plt(nkp,xkl,kl,ef)
        ...
        write(99,'(a,f12.6,a,f12.6,a)') 'set xrange [ -1 : 1]'
        write(99,'(a)') &
